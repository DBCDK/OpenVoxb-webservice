...
#
    ...
    function deleteUser($params) {
        ...
        try {
            $this->oci->set_query("UPDATE voxb_users SET disabled=1 WHERE userId=$userId AND institutionId=$institutionId");
        } catch (ociException $e) {
    ...
    function fetchData($params) {
        ...
        verbose::log(DEBUG, "Er i fetchData");
        ...
        verbose::log(DEBUG, "skal kalde voxb_items");
    ...
    function fetchUser($params) {
        ...
        if (!empty($params->userId->_value)) {  // userId given in input
            ...
            if (empty($aFv->userIdentifierValue->_value) || empty($aFv->userIdentifierType->_value) || empty($aFv->identityProvider->_value) || empty($aFv->institutionId->_value)) {
            ...
            foreach ($data as $uData) {
                if (($uData['USERIDENTIFIERVALUE'] == $userIdentifierValue) and ( $uData['USERIDENTIFIERTYPE'] == $aFv->userIdentifierType->_value) and ( $uData['IDENTITYPROVIDER'] == $aFv->identityProvider->_value) and ( $uData['INSTITUTIONID'] == $aFv->institutionId->_value)) {
                    $found = true;
                    break;
                }
            }
