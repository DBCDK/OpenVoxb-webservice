...
function update_voxb($userid, $reviewdata, $rating, $title, $contributor, $type, $year, $id, $idtype) {
    global $UpdateMyData_org;
    $xml = str_replace('$userid', $userid, $UpdateMyData_org);
    $xml = str_replace('$reviewdata', $reviewdata, $xml);
    $xml = str_replace('$rating', $rating, $xml);
    $xml = str_replace('$title', $title, $xml);
    $xml = str_replace('$contributor', $contributor, $xml);
    $xml = str_replace('$type', $type, $xml);
    $xml = str_replace('$year', $year, $xml);
    $xml = str_replace('$id', $id, $xml);
    $xml = str_replace('$idtype', $idtype, $xml);
    echo $xml;
    exit;
}
...
function get_user_id($email) {
    ...
    $q = "select userid from voxb_users where useridentifiervalue=:email and institutionid = 870970";
 	  ...
 	  <soapenv:Body>
 	     <voxb:createUserRequest>
 	        ...
 	        <voxb:authenticationFingerprint>
 	           ...
 	           <!--Optional:-->
						<voxb:institutionId>870970</voxb:institutionId>
 	        </voxb:authenticationFingerprint>
 	     </voxb:createUserRequest>
 	  </soapenv:Body>
	</soapenv:Envelope>';
...
//            exit;
    ...
    if (isset($array['ucd']) && !empty($userid)) {
        foreach ($array['ucd'] as $id => $array2) {
            ...
            if (isset($array2['rating']))
                $rating = $array2['rating'];
            if (isset($array2['reviewdata']))
                $reviewdata = $array2['reviewdata'];

            // try to find a valid identificator from id
            list ($lib, $newid) = explode(':', $id);

            $faust = materialId::validateFaust($newid);
            if ($faust) {
                $idtype = 'FAUST';
                $id = $faust;
            } else {
                $ean = materialId::validateEAN($id);
                if ($ean) {
                    $idtype = 'EAN';
                    $id = $ean;
                } else {
                    $isbno = materialId::validateISBN($id);
                    if ($isbno) {
                        $idtype = 'ISBN';
                        $id = $isbno;
                    } else {
                        $issn = materialId::validateISSN($id);
                        if ($issn) {
                            $idtype = 'ISSN';
                            $id = $issn;
                        }
            ...
            $obj = bibdk_ws($orgid, $bibdk_ws_url);
            ...
            if ($obj != FALSE) {
                ...
                if (check_existing_item($id, $idtype, $userid)) {
                    // update voxb with the correct data
//                    update_voxb($userid, $reviewdata, $rating, $title, $contributor, $type, $year, $id, $idtype);
...
//echo "\n--- CREATE MYDATA REQUEST ---------------------------------------------\n";
            ...
            $curl->set_post_xml($xml);
            $res = $curl->get($voxb_ws_url);
            if (!preg_match("/faultstring|error|Error|FATAL/", $res)) {
                echo "Created item userid: $userid id: $id idtype: $idtype, voxb_url:$voxb_ws_url\n";
            } else {
                // her skal vi lave en update
                echo "Error in creating item userid: $userid id: $id idtype: $idtype voxb_ws_url:$voxb_ws_url\n";
                echo "\n---------------------------------------------------------------------\n";
                echo $xml;
                echo "\n---------------------------------------------------------------------\n";
                echo $res;
