...
class voxb_items {
    ...
    private function itemColumns() {
        $columns = array('u.USERID', 'i.OBJECTID', 'i.RATING', 'i.CREATION_DATE', 'i.MODIFICATION_DATE', 'i.DISABLED', 'i.ITEMIDENTIFIERVALUE');
        return $columns;
    }
    ...
    private function getItemsFromIds($itemIds) {
        ...
        try {
            $sql = 'SELECT ' . $columns . ' from ' . $tables . ' WHERE i.ITEMIDENTIFIERVALUE in (' . implode(',', $itemIds) . ') AND ' . $filter;
    ...
    private function getItemsFromObjectIds($object_array) {
        foreach ($object_array as $objectIds) {
            ...
            try {
                $sql = 'SELECT ' . $columns . ' from ' . $tables . ' where i.objectid in (' . implode(',', $ids) . ') AND ' . $filter;
                $this->oci->set_query($sql);
                while ($row = $this->oci->fetch_into_assoc()) {
                    $item_id = $row['ITEMIDENTIFIERVALUE'];
